...
// A real .env carries STORE_API_URL three times — localhost under a docker
...
func TestPreferLocalhost(t *testing.T) {
	all := ParseEnvFileAll("STORE_API_URL=http://localhost:3000\nSTORE_API_URL=http://store-api\nOTHER=x\nOTHER=y")

	if got := all["STORE_API_URL"]; len(got) != 2 {
		t.Fatalf("ParseEnvFileAll kept %v, want both", got)
	}

	picked := PreferLocalhost(all)
	if picked["STORE_API_URL"] != "http://localhost:3000" {
		t.Errorf("STORE_API_URL = %q, want the localhost value even though it is shadowed", picked["STORE_API_URL"])
	}
	if picked["OTHER"] != "y" {
