...
in the checkout after mise and before the install, over the `.env*` and `.local*.env` files
crew copied in. It must write files, not print values: its output is logged with the
install's.
